...
**Bulk-flagging blank certificates as N/A:** run `previewBlankCertsNa()` to read the counts in the execution log, then `applyBlankCertsNa()` to write. It flags `cert_<key>_na = TRUE` on every blank expiry for `ra`, `fa`, `ff`, `ec`, `ppe`, `lifting`, `scaffolding`, across both teams and archived rows alike, intersected with the row's applicable cert set. `wah_practical`, `wah_theoretical` and `mcu` are excluded by design — a blank on one of those usually means "not entered yet", and a blank medical flagged N/A would settle the RDT eligibility question by decision rather than by data. A cert suspended by hand is left alone and reported. Idempotent, and reversible two ways: unticking the box on the form, since the flag never rewrites the date, or `revertBlankCertsNa()`, which undoes the last run cell for cell from a record kept in script properties. That record survives one run — a second apply overwrites it, and a revert clears it. It does not stamp `updated_at` / `updated_by`: rule 8 governs writes made on behalf of a session, and stamping the whole roster would erase the only signal the "Recently updated" panel has.
